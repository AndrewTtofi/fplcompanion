  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <h3 className="text-xl font-bold text-gray-900 mb-4">Compare with Opponent</h3>
        ...
        <form onSubmit={handleCompare} className="mb-6">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-fpl-purple focus:border-transparent"
            />
            <button
              type="submit"
                ...
                {leagueData.standings.results
                  ...
                  .map((team) => (
                    <button
                      ...
                      className="w-full bg-gray-50 hover:bg-fpl-purple hover:bg-opacity-10 border border-gray-200 rounded-lg p-3 text-left transition"
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="font-medium text-gray-900">{team.entry_name}</div>
                          <div className="text-sm text-gray-600">{team.player_name}</div>
                        </div>
                        <div className="text-right">
                          <div className="text-sm text-gray-500">Rank {team.rank}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="flex items-center justify-center">
          <div className="text-center">
            ...
            <div className="text-sm text-gray-500 mt-2">GW Difference</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {comparison.shared_players.map((player) => (
            <div key={player.id} className="bg-blue-50 border border-blue-200 rounded-lg p-3">
              ...
              <div className="text-sm text-blue-700">{player.team}</div>
              <div className="text-lg font-bold text-blue-900 mt-1">{player.points} pts</div>
            </div>
          ))}
        </div>
      </div>
...
function TeamCard({ team, label, highlight = false }) {
  return (
    <div className={`bg-white rounded-lg border-2 p-4 ${
      highlight ? 'border-fpl-purple shadow-lg' : 'border-gray-200'
    }`}>
      ...
      <div className="mt-4 space-y-2">
        <div className="flex justify-between">
          <span className="text-sm text-gray-600">GW Points:</span>
          <span className="font-bold text-fpl-purple">{team.gameweek_points}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-sm text-gray-600">Net Points:</span>
          <span className="font-bold">{team.net_points}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-sm text-gray-600">Overall:</span>
          <span className="font-medium">{team.overall_points}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-sm text-gray-600">OR:</span>
          <span className="font-medium">{team.overall_rank?.toLocaleString()}</span>
        </div>
      </div>
    </div>
  );
...
function CaptainCard({ name, points, multiplied, label }) {
  return (
    <div className="bg-yellow-50 border border-yellow-300 rounded-lg p-4">
      ...
      <div className="font-bold text-yellow-900">{name || 'N/A'}</div>
      {points !== undefined && (
  ...
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 flex items-center justify-between">
      ...
      <div className="text-right">
        ...
        {player.is_captain && (
          <div className="text-xs text-gray-500">{player.points} × 2</div>
        )}
      </div>
    </div>
  );
